...
import createDependencyContainer, { AppDependencies } from "../lib/di";
...
export const isAuth = async (req: RequestWithUser, res: Response, next) => {
    ...
    db.execute("SELECT * FROM users WHERE username = ?", [payload?.username], (err: QueryError, result: RowDataPacket[]) => {
        if (err) {
            console.log(err);
